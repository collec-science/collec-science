...
class ImportObject
{
    ...
    function initFile($filename, $separator = ",", $utf8_encode = false)
    {
        ...
        if ($this->handle = fopen($filename, 'r')) {
            ...
            for ($range = 0; $range < count($data); $range++) {
                ...
                if (in_array($value, $this->fields) || substr($value, 0, 3) == "md_") {
                    $this->fileColumn[$range] = $value;
                } else {
                    throw new PpciException(sprintf(_('La colonne %1$s n\'est pas reconnue (%2$s)'), $range, $value));
                }
            }
        } else {
    ...
    function importAll()
    {
        ...
        $this->movement->autoFormatDate = false;
        /**
        ...
        while (($data = $this->readLine()) !== false) {
            ...
            if (!empty($values["sample_identifier"])) {
                ...
                if (!empty($values["sample_metadata_json"])) {
                    ...
                    if (json_last_error() != JSON_ERROR_NONE) {
                        throw new PpciException(sprintf(_("Ligne %s : le décodage du champ JSON sample_metadata_json n'a pas abouti"), $num));
                    }
                } else {
                    $md_array = array();
                }
                ...
                try {
                     ...
                     */
                    if (!empty($values["composite_parents_uid"]) || !empty($values["composite_parents_identifier"])) {
                        ...
                        foreach ($parentsUid as $puid) {
                             ...
                             */
                            $this->subSample->addSubsample($parentData["sample_id"], $values["composite_multiple_value"], 2, null, $sample_id);
                        }
                    }
                    ...
                    throw new PpciException("Line $num : error when import sample - " . $pe->getMessage());
                }
            }
            /**
            ...
            if (empty($values["container_parent_uid"]) && !empty($values["container_parent_identifier"])) {
                ...
                if (empty($values["container_parent_uid"])) {
                    throw new PpciException("Line $num : the container " . $values["container_parent_identifier"] . " don't exists into the database");
                }
            }
            /**
            ...
            if (!empty($values["container_identifier"])) {
                ...
                try {
                    ...
                    throw new PpciException("Line $num : error when import container - " . $pe->getMessage());
                }
                /**
                ...
                if (!empty($values["container_parent_uid"])) {
                    try {
                        ...
                        throw new PpciException("Line $num : error when create input movement for container - " . $e->getMessage());
                    }
                }
            }
            /**
            ...
            if ($sample_uid > 0 && $container_uid > 0) {
                try {
                    ...
                    throw new PpciException("Line $num : error when create input movement for sample (" . $e->getMessage() . ")");
                }
            }
        ...
        if (!empty($data["sample_identifier"])) {
            ...
            foreach ($fieldDates as $fieldDate) {
                if (!empty($data[$fieldDate])) {
                    ...
                    strlen($data[$fieldDate]) > 10 ? $mask = $_SESSION["MASKDATELONG"] : $mask = $_SESSION["date"]["maskdate"];
                    $result = date_parse_from_format($mask, $date);
    ...
    function initIdentifiers()
    {
        if (!$this->initIdentifiers) {
            ...
            foreach ($dit as $value) {
                $this->fields[] = $value["identifier_type_code"];
     ...
     */
    function importExterneExec($data,  $sic, $post)
    {
        $simpleFields = array(
            "identifier",
            "wgs84_x",
        ...
        foreach ($data as $row) {
            ...
            foreach ($row as $fieldname => $fieldvalue) {
                if (substr($fieldname, 0, 3) == "md_" && strlen($fieldvalue) > 0) {
                    ...
                    if (!array_key_exists($colname, $metadata)) {
                        ...
                        /**
                         * Add a control to search if it's a multivalue field or not
                         */
                        $md_col_array2 = explode(", ", $fieldvalue);
                        if (count($md_col_array) > 1 && !count($md_col_array2) > 1) {
                            // It's seem an array
            ...
            try {
                ...
                if ($uid > 0) {
                    ...
                    throw new PpciException("Problem when importing - line " . $this->nbTreated + 1);
                }
            } catch (\Exception $e) {
