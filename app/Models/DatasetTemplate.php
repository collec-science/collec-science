...
class DatasetTemplate extends PpciModel
{
    ...
    function generateData(int $id, string $uids): array
    {
        ...
        switch ($ddataset["dataset_type_id"]) {
            ...
            case 2:
                ...
                $this->collection->autoFormatDate = false;
    ...
    function formatData(array $dbdata): array
    {
        ...
        if (empty($ddataset)) {
            throw new PpciException(_("Le modèle d'export n'a pas été correctement initialisé"));
        }
        if (!is_object($this->datasetColumn)) {
            $this->datasetColumn = new DatasetColumn;
        }
    ...
    function getTemplateFromName(string $name): array
    {
        ...
        if ($res["dataset_template_id"] > 0) {
            ...
            throw new PpciException(sprintf(_("Le modèle %s n'existe pas"), $name));
        }
    }
    /**
    ...
    function duplicate(int $id)
    {
        ...
        if (empty($data["dataset_template_id"])) {
            throw new PpciException(_("Impossible de lire le modèle à dupliquer"));
        }
        /**
