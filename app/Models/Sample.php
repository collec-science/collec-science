...
class Sample extends PpciModel
{
     ...
     * @see ObjetBDD::lire()
     */
    public function read($uid, $getDefault = true, $parentValue = null): array
    {
    ...
    public function readFromId($sample_id)
    {
    ...
    public function write($data): int
    {
        ...
        if ($ok) {
            ...
            if ($uid > 0) {
                ...
                throw new PpciException(_("Un problème est survenu lors de l'écriture de l'objet"));
            }
        } else {
            throw new PpciException(_("Vous ne disposez pas des droits pour modifier cet échantillon"));
        }
    }
    ...
    private function _generateSearch(array $param)
    {
        if (empty($this->where) || ($param != $this->paramSearch)) {
            ...
            if ($searchOk) {
                 ...
                 */
                if (($_SESSION["consultSeesAll"] == 1 || $_SESSION["userRights"]["gestion"] == 1) && !empty($param["metadata_field"][0]) && strlen($param["metadata_value"][0]) > 0) {
                    $where .= $and . " ";
                    /**
    ...
    private function _executeSearch(string $sql, array $data, $metadatafilter = ""): array
    {
         ...
         */
        unset($this->fields["movement_date"]);
        unset($this->fields["borrowing_date"]);
        unset($this->fields["expected_return_date"]);
        unset($this->fields["change_date"]);
        return $list;
    ...
    public function getForExport($uids)
    {
        if (empty($uids)) {
            ...
            if (count($data) > 0) {
                ...
                throw new PpciException(_("Droits insuffisants pour exporter les échantillons"));
            }
        }
    }
    ...
    public function verifyBeforeImport($row)
    {
        if (count($row) > 0) {
            if (empty($row["dbuid_origin"])) {
                throw new PpciException(_("L'identifiant de la base de données d'origine n'a pas été fourni"));
            }
            /*
             * Verification de l'existence de la collection
             */
            if (empty($row["collection_name"])) {
                throw new PpciException(_("Le nom de la collection n'a pas été renseigné"));
            }
            /**
            ...
            if (!empty($row["uid"])) {
                if (!$this->is_uniqueByCollectionName($row["uid"], $row["identifier"], $row["collection_name"])) {
                    throw new PpciException(_("L'identifiant de l'échantillon existe déjà dans la collection"));
                }
            }
            /*
            ...
            if (empty($row["sample_type_name"])) {
                throw new PpciException(_("Le type d'échantillon n'a pas été renseigné"));
            }
            /*
            ...
            foreach ($fieldDates as $fieldDate) {
                if (!empty($row[$fieldDate])) {
                     ...
                     */
                    $result = date_parse_from_format($_SESSION["date"]["maskdate"], $row[$fieldDate]);
                    if ($result["warning_count"] > 0) {
                        ...
                        if ($result["warning_count"] > 0) {
                            throw new PpciException(sprintf(_("Le format de date de %s n'est pas reconnu. "), $fieldDate));
                        }
                    }
                    if ($result["warning_count"] == 0) {
                        /*
                        ...
                        if (strlen($result["year"]) == 0 || strlen($result["month"]) == 0 || strlen($result["day"]) == 0) {
                            throw new PpciException(sprintf(_("La date de %s est incomplète ou incohérente. "), $fieldDate));
                        }
                    }
                }
            }
            ...
            foreach ($numFields as $numField) {
                if (strlen($row[$numField]) > 0 && !is_numeric($row[$numField])) {
                    throw new PpciException(sprintf(_("Le champ %s n'est pas numérique"), $numField));
                }
            }
            /**
            ...
            foreach (array("country_code", "country_origin_code") as $field) {
                if (!empty($row[$field])) {
                    ...
                    if (!$country_id > 0) {
                        throw new PpciException(sprintf(_("Le code de pays %s n'est pas connu"), $row[$field]));
                    }
                }
            }
        }
