...
<script>
	$(document).ready(function () {
		...
		function initializeBootstrapTooltip() {
			var tooltipTriggerList = [].slice.call(document.querySelectorAll('.container'))
				...
				.done(function (d) {
					if (d) {
						...
						for (var lst = 0; lst < containers.length; lst++) {
							...
							row += '</td>';
							row += '<td title="">';
							row += '<a class="" data-uid="' + containers[lst].uid + '" href="containerDisplay?uid=' + containers[lst].uid + '" title="">';
		...
		<table id="containerList" class="table table-bordered table-hover display">
			...
			<tbody class="nowrap">
				...
				<tr>
					...
					{/if}
					<td class="center">
						<a href="containerDisplay?uid={$containers[lst].uid}"  class="container tooltiplink"  data-uid="{$containers[lst].uid}" data-bs-toggle="tooltip-container" title="{t}Consultez le détail{/t}">
							{$containers[lst].uid}
						</a>
						<img class="plus hover" id="{$containers[lst].uid + 9000000}" data-uid="{$containers[lst].uid}" src="display/images/plus.png" height="15">
					</td>
					<td>
						<a id="identifier{$containers[lst].uid}" href="containerDisplay?uid={$containers[lst].uid}"  title="{t}Consultez le détail{/t}">
